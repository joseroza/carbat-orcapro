...
const db = require("../db");
const { encryptCliente, decryptCliente } = require("../middleware/crypto.middleware");

router.get("/", async (req, res) => {
  try {
    const { rows } = await db.query("SELECT * FROM clientes ORDER BY created_date DESC");
    res.json(rows.map(decryptCliente));
...
router.get("/:id", async (req, res) => {
  try {
    ...
    res.json(decryptCliente(rows[0]));
  } catch (e) { res.status(500).json({ error: e.message }); }
});

router.post("/", async (req, res) => {
  try {
    const f = encryptCliente(req.body);
