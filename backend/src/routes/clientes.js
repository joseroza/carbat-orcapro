...
router.post("/", async (req, res) => {
  try {
    ...
    const { rows } = await db.query(
      `INSERT INTO clientes (razao_social,nome_fantasia,cnpj,email,telefone,endereco,cidade,estado,contato_principal)
  ...
  try {
    ...
    const { rows } = await db.query(
      `UPDATE clientes SET razao_social=$1,nome_fantasia=$2,cnpj=$3,email=$4,telefone=$5,
       endereco=$6,cidade=$7,estado=$8,contato_principal=$9,updated_date=NOW()
       WHERE id=$10 RETURNING *`,
      [f.razao_social,f.nome_fantasia,f.cnpj,f.email,f.telefone,f.endereco,f.cidade,f.estado,f.contato_principal,req.params.id]
    );
    res.json(rows[0]);
  } catch (e) { res.status(500).json({ error: e.message }); }
});
