...
const db = require("../db");
const { encryptRomaneio, decryptRomaneio } = require("../middleware/crypto.middleware");
...
router.get("/", async (req, res) => {
  try {
    ...
    res.json(rows.map(r => decryptRomaneio(formatRow(r))));
...
router.get("/:id", async (req, res) => {
  try {
    ...
    res.json(decryptRomaneio(formatRow(rows[0])));
  } catch (e) { res.status(500).json({ error: e.message }); }
});

router.post("/", async (req, res) => {
  try {
    const f = encryptRomaneio(req.body);
    const { rows } = await db.query(
      ...
      [f.numero, f.proposta_id||null, f.proposta_numero, f.cliente_nome,
       ...
       f.status||"pendente", f.endereco_entrega, f.observacoes, JSON.stringify(req.body.itens||[])]
    );
    res.json(decryptRomaneio(formatRow(rows[0])));
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
router.put("/:id", async (req, res) => {
  try {
    ...
    const { rows } = await db.query(
      ...
      [f.numero, f.proposta_id||null, f.proposta_numero, f.cliente_nome,
       safeDate(f.data_emissao), safeDate(f.data_entrega),
       f.status, f.endereco_entrega, f.observacoes, JSON.stringify(req.body.itens||[]), req.params.id]
    );
    res.json(decryptRomaneio(formatRow(rows[0])));
  } catch (e) { res.status(500).json({ error: e.message }); }
});
